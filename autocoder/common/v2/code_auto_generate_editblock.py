    ...
    def single_round_run(
        ...
        if self.args.system_prompt and self.args.system_prompt.strip() == "claude":
            conversations.append(
                {
                    "role": "system",
                    "content": sys_prompt.claude_sys_prompt.prompt()
                    + "\n===\n"
                    + init_prompt,
                }
            )
            ...
            conversations.append({"role": "system", "content": init_prompt})
            ...
            with ThreadPoolExecutor(
                ...
                for llm in self.llms:
                    ...
                    for _ in range(self.generate_times_same_model):
                        ...
                        if count == 0:
                            logger.info(
                                f"code generation with model(Stream): {model_name}"
                            )
                            ...
                            logger.info(
                                f"code generation with model(Non-stream): {model_name}"
                            )
                            futures.append(
                                executor.submit(
                                    chat_with_continue,
