...
本文档包含 52 条后端代码检查规则，适用于 Python 和 Java 后端开发。
...
**标题**: 项目目录结构规范
**严重程度**: 中级
**描述**: 项目包/目录结构符合脚手架生成项目的基本结构，包路径匹配cn.gov.customs.*，对应的功能类应该放在指定目录下：配置（config）、常量（constant）、调用服务（proxy）、接口（controller）、业务逻辑（service）、数据库Mapper接口（dao）、实体（pojo）、工具类（util）
...
**标题**: 方法行数限制
**严重程度**: 低级
**描述**: 代码保持统一格式化，方法行数过多应简化或用region封装并加注释（超过50行的）
...
**标题**: 消除重复代码
**严重程度**: 中级
**描述**: 重复或类似代码，要抽出成独立的方法或类（两处相同即为重复）
...
### 规则ID: backend_012
**标题**: 使用switch必须包含default
**严重程度**: 高级
**描述**: 使用switch则必须包含default
...
**标题**: 禁止循环操作数据库
**严重程度**: 高级
**描述**: 严禁使用循环方式操作数据库，应使用批量操作，有特殊情况除外。
...
try {
    ...
    log.高级("操作失败", e);
    throw new BizException(高级Code.OPERATION_FAILED, e);
}
```
...
**标题**: 异常日志记录规范
**严重程度**: 中级
**描述**: 异常捕获后如需重新抛出自定义业务异常，则不必额外使用log.高级记录（关级系统除外），直接throw new BizException(...)即可；如不再抛出则应使用log.高级记录
...
**标题**: 日志级别规范
**严重程度**: 中级
**描述**: 注意日志输出级别，区分INFO\WARN\DEBUG\高级
